...
def compute_metrics(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    tmax = inputs.get("tmax_defl_ms")
    mdi = float("nan") if (is_bad_number(tmax) or float(tmax) <= 0) else safe_ratio(tmax, inputs["qrs_pvc_ms"])
...
def interpret_mdi(mdi: float) -> Tuple[str, str]:
    if is_bad_number(mdi):
        return "Insufficient data", "Enter time to maximal deflection and QRS duration to compute MDI."
    if mdi >= 0.55:
        return "MDI ≥0.55 → epicardial origin more likely", "MDI is a commonly cited marker; values at or above ~0.55 have been associated with epicardial origin in some idiopathic ventricular arrhythmias."
    return "MDI <0.55 → endocardial origin more likely", "MDI below the commonly cited ~0.55 cutoff has been associated with endocardial origin in some idiopathic ventricular arrhythmias."
...
tmax_defl_ms = st.sidebar.number_input(
    ...
    step=1,
    key="tmax_defl_ms",
    help="Shortest interval from QRS onset to maximal positive or negative deflection in any precordial lead. Used to compute maximal deflection index (MDI) = tmax / QRS duration.",
)
...
mdi_title, mdi_note = interpret_mdi(m["mdi"])
...
metric_cols[5].metric("MDI", fmt(m["mdi"]))
...
with colC:
    ...
    bullets = []
    bullets.append(f"Inputs: BBB={bbb}; axis={axis}; lead I={lead1}; transition PVC={tz_pvc}; transition SR={tz_sr}; QRSd={qrs_pvc_ms} ms.")
    if not is_bad_number(m["mdi"]):
        bullets.append(f"MDI = {fmt(m['mdi'])}. Commonly cited cutoff: ≥0.55 suggests epicardial origin; <0.55 suggests endocardial origin (context-dependent).")
    if bbb == "LBBB" and axis == "Inferior":
        bullets.append("Morphology (LBBB) with inferior axis is compatible with an outflow tract pattern; use transition, lead I, and indices for RVOT vs LVOT estimation.")
    elif bbb == "RBBB":
        bullets.append("RBBB morphology suggests a left ventricular origin is more likely, depending on axis and other features.")
    yamada_text = "\n\n".join(bullets)
...
with colD:
    ...
    steps = []
    steps.append(f"Step 1 (regionalization): BBB={bbb}; axis={axis}; lead I={lead1}.")
    if bbb == "LBBB" and axis == "Inferior":
        steps.append("Step 2 (OT pattern): assess precordial transition and apply OT indices.")
        steps.append(f"Transition: PVC={tz_pvc}; SR={tz_sr}; TZ index={m['tz_index']:+d}.")
        if m["tzp"] <= 2:
            steps.append("Early transition (≤V2) supports a left-sided OT estimate in many idiopathic OT-VAs.")
        elif m["tzp"] >= 4:
            steps.append("Late transition (≥V4) supports a right-sided OT estimate in many idiopathic OT-VAs.")
        else:
            steps.append("Borderline transition (V3): integrate V2 transition ratio and V2S/V3R.")
        if yoshida_title != "Insufficient data":
            steps.append(f"V2S/V3R: {fmt(m['v2s_v3r'])} → {yoshida_title}.")
        if bet_applicable:
            steps.append(f"V2 transition ratio: {fmt(m['v2_transition_ratio'])} → {bet_title}.")
        steps.append(f"ASC indices (Ouyang): {ouy_title}.")
    else:
        steps.append("Step 2: pattern is not classic LBBB with inferior axis; interpret OT-specific indices cautiously and consider broader anatomic differentials.")
    if not is_bad_number(m["mdi"]):
        steps.append(f"MDI context: {mdi_title}.")
    enriquez_text = "\n\n".join(steps)
...
rows.append({
    ...
    "Metric(s)": f"BBB={bbb}; axis={axis}; lead I={lead1}; TZ PVC={tz_pvc}; MDI={fmt(m['mdi'])}",
...
rows.append({
    ...
    "Metric(s)": f"Stepwise: BBB={bbb}; axis={axis}; TZ PVC={tz_pvc}; V2S/V3R={fmt(m['v2s_v3r'])}",
...
payload = {
    ...
    "metrics": {
        ...
        "max_deflection_index": None if is_bad_number(m["mdi"]) else float(m["mdi"]),
...
with st.expander("References (links)"):
    ...
    st.markdown("- Yamada 2019 (PubMed): https://pubmed.ncbi.nlm.nih.gov/31502322/")
    st.markdown("- Muser 2021 stepwise approach (PMC): https://pmc.ncbi.nlm.nih.gov/articles/PMC8534438/")
    st.markdown("- MDI definition and cutoff example (PDF): https://jag.journalagent.com/ejm/pdfs/EJM_20_2_115_123.pdf")
    st.markdown("- Chun 2007 (abstract; RVOT pattern): https://pubmed.ncbi.nlm.nih.gov/17541380/")
